...
class CABSwitch(app_manager.RyuApp):
    ...
    def handle_ip(self,datapath,msg,in_port):
        ...
        # request_str = str(ip_src)+'\t'+str(ip_dst)+'\t'+str(eth.src)+'\t'+str(eth.dst)+'\n'
        ...
        # If there is more than one query hit the same bucket before the bucket been installed,
        # only need to flowmod the bucket once.
        bucket = rules[0]
        bucket_str = ipv4_to_str(bucket.ip_src)+'/'+ipv4_to_str(bucket.ip_src_mask)+'\t'+\
                ipv4_to_str(bucket.ip_dst)+'/'+ipv4_to_str(bucket.ip_dst_mask)+'\t'+\
                eth_to_str(bucket.port_src)+'/'+eth_mask_to_str(bucket.port_src_mask)+'\t'+\
                eth_to_str(bucket.port_dst)+'/'+eth_mask_to_str(bucket.port_dst_mask)
        ...
        for rule in rules[1:]:
            ...
            inst = [parser.OFPInstructionActions(ofproto.OFPIT_APPLY_ACTIONS,actions)]
            self.add_flow(datapath,1, rule.priority,match,inst,ofproto.OFP_NO_BUFFER,timeout)

            self.logger.debug( "install flow %s %s %s %s %s %s %s %s",
                    ipv4_to_str(rule.ip_src), ipv4_to_str(rule.ip_src_mask), 
                    ipv4_to_str(rule.ip_dst), ipv4_to_str(rule.ip_dst_mask),
                    eth_to_str(rule.port_src), eth_mask_to_str(rule.port_src_mask),
                    eth_to_str(rule.port_dst), eth_mask_to_str(rule.port_dst_mask))
        ...
        self.logger.debug( "install bucket %s %s %s %s %s %s %s %s",
                ipv4_to_str(bucket.ip_src), ipv4_to_str(bucket.ip_src_mask), 
                ipv4_to_str(bucket.ip_dst), ipv4_to_str(bucket.ip_dst_mask),
                eth_to_str(bucket.port_src), eth_mask_to_str(bucket.port_src_mask),
                eth_to_str(bucket.port_dst), eth_mask_to_str(bucket.port_dst_mask))
