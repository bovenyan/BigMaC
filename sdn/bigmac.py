# Copyright (C) 2014 High Speed Network Labtory, Polytechnic
...
class CABSwitch(app_manager.RyuApp):
    ...
    def error_msg_handler(self, ev):
        ...
        self.logger.info('OFPErrorMsg received: type=0x%02x \
                         code=0x%02x message=%s', msg.type, msg.code,
    ...
    def _packet_in_handler(self, ev):
        ...
        # datapath is the object which denotes the link
    ...
    def handle_ip(self, datapath, msg, in_port):
        ...
        request = pkt_h(ipv4_to_int(ip_src),
                        ipv4_to_int(ip_dst), src_port, dst_port)

        # request_str = str(ip_src)+'\t'+
        # str(ip_dst)+'\t'+str(eth.src)+'\t'+str(eth.dst)+'\n'
        ...
        # If there is more than one query hit the same
        # bucket before the bucket been installed,
