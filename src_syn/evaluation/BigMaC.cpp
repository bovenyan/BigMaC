...
int bigmac::check_tags(){
    ...
    for (int sRuleID = 0; sRuleID < bs_sRule_rec.size(); ++sRuleID){ // check every sRule;
        sRule_rec sRule = bs_sRule_rec[sRuleID];

        if (sRule.active){ 
            set<int> assoc_ns;

            for (const int & nodeID : sRule.path){ // check every node;
                const map<int, double> & Entry = switch_rec[nodeID].nsEntries;

                for (auto iter = Entry.begin(); iter != Entry.end(); ++iter){
                    if (rList->list[iter->first].dep_rule(rList->list_switching[sRuleID])){
